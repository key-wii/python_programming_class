...
class Shell(GameObject):
    ...
    '''
    def __init__(self, coord, vel, player = 0, rad=20, color=None):
        '''
        ...
        self.rad = rad
        self.is_alive = True
        self.player = player #keeps track of who shot the shell
...
class EnemyShell(GameObject):
    '''
    The enemy ball class. Creates a ball, controls it's movement and implement it's rendering.
    '''
...
class Cannon(GameObject):
    ...
    def strike(self):
        ...
        if self.counter == 1: #rotates through which ball to shoot
            ball = Shell(list(self.coord), [int(vel * np.cos(angle)), int(vel * np.sin(angle))], 1)
            self.counter += 1
        elif self.counter == 2: #shoots the second ball
            ball = Shell2(list(self.coord), [int(vel * np.cos(angle)), int(vel * np.sin(angle))], 1)
            self.counter += 1
        elif self.counter == 3: #shoots third ball and resets back to first
            ball = Shell3(list(self.coord), [int(vel * np.cos(angle)), int(vel * np.sin(angle))], 1)
            self.counter = 1
    ...
    def check_collision(self, ball):
        ...
        min_dist = 40 + ball.rad
        return dist <= min_dist
    
    #get which cannon this is
    def get_player(self):
        return 1
...
class Cannon2(GameObject):
    ...
    def strike(self):
        ...
        angle = self.angle
        if self.counter == 1: #rotates through which ball to shoot
            ball = Shell(list(self.coord), [int(vel * np.cos(angle)), int(vel * np.sin(angle))], 2)
            self.counter += 1
        elif self.counter == 2: #shoots the second ball
            ball = Shell2(list(self.coord), [int(vel * np.cos(angle)), int(vel * np.sin(angle))], 2)
            self.counter += 1
        elif self.counter == 3: #shoots third ball and resets back to first
            ball = Shell3(list(self.coord), [int(vel * np.cos(angle)), int(vel * np.sin(angle))], 2)
            self.counter = 1
        self.pow = self.min_pow
        self.active = False
        return ball
...
class Manager:
    ...
    def process(self, events, screen):
        ...
        if pg.mouse.get_focused():
            ...
            #if guns exist
            if len(self.guns) == 2:
                self.guns[0].set_angle(mouse_pos)
                self.guns[1].set_angle(mouse_pos)
        ...
        for event in events:
            if event.type == pg.QUIT:
                ...
                if event.key == pg.K_UP:
                    self.guns[0].moveV(-15)
                elif event.key == pg.K_DOWN:
                    self.guns[0].moveV(15)
                if event.key == pg.K_LEFT:
                    self.guns[0].moveH(-15)
                elif event.key == pg.K_RIGHT:
                    self.guns[0].moveH(15)
            elif event.type == pg.MOUSEBUTTONDOWN:
                if event.button == 1:
                    self.guns[0].activate()
            ...
            if event.type == pg.KEYDOWN:
                if event.key == pg.K_w:
                    self.guns[1].moveV(-15)
                elif event.key == pg.K_s:
                    self.guns[1].moveV(15)
                if event.key == pg.K_a:
                    self.guns[1].moveH(-15)
                elif event.key == pg.K_d:
                    self.guns[1].moveH(15)
            if event.type == pg.KEYDOWN and event.key == pg.K_SPACE:
                self.guns[1].activate()
    ...
    def collide(self):
        ...
        for i, ball in enumerate(self.balls):
            for j, gun in enumerate(self.guns):
                if gun.check_collision(ball) and gun.get_player() != ball.get_player():
                    guns_c.append(j)
        
        for j in reversed(guns_c):
            self.guns.pop(j)
